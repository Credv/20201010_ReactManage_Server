  ...
  UserModel.findOne({ username })
    .then(user => {
      ...
      if (user) {
        ...
        // 保存
        return UserModel.create({ ...req.body, password: md5(password || '3177102530') })
      }
    })
    .then(user => {
      // 返回包含user的json数据
...
//删除分类
router.post('/manage/category/del', (req, res) => {
...
//删除产品
router.post('/manage/product/del', (req, res) => {
  const { _id } = req.body
...
router.get('/manage/product/findOne', (req, res) => {
  ...
  ProductModel.findOne({ _id: id })
    ...
    .catch(error => {
      console.error('获取单独产品信息异常', error)
      res.send({ status: 1, msg: '获取单独产品信息异常, 请重新尝试' })
    })
})
// 搜索产品列表
